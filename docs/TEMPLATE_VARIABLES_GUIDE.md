...
> **⚠️ УСТАРЕЛО (deprecated).** Синтаксис `{{переменная}}` из этого документа больше не используется.
> Актуальный справочник тегов — в приложении на странице **«Шаблоны → Справочник тегов»** (`/templates/guide`).
> Действующий синтаксис: одинарные фигурные скобки `{tag}` (docxtemplater), циклы `{#items}…{/items}`.
